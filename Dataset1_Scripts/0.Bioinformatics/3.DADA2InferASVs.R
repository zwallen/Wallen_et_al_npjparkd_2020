...
filtpath <- "../../Script_Output/Dataset1_Output/PrimerTrimmedSeqs/QualityFiltSeqs"

filtFs <- list.files(filtpath, pattern="_R1_001.fastq.gz", full.names=TRUE)
filtRs <- list.files(filtpath, pattern="_R2_001.fastq.gz", full.names=TRUE)
...
#Forward
pdf("../../Script_Output/Dataset1_Output/ErrorRatePlotF.pdf")
plotErrors(errF, nominalQ=TRUE)
dev.off()
#Reverse
pdf("../../Script_Output/Dataset1_Output/ErrorRatePlotR.pdf")
...
cat("\n","Fraction of chimeras:", sum(seqtab.nochim)/sum(seqtab2), "\n")

#write chimera free table to disk
saveRDS(seqtab.nochim, "../../Script_Output/Dataset1_Output/seqtab.rds")

#write ASV table
seqtab.df <- add_rownames(as.data.frame(seqtab.nochim), "SampleID")
write.table(seqtab.df, "../../Script_Output/Dataset1_Output/seqtab.txt", sep="\t", row.names=F, quote=F)
