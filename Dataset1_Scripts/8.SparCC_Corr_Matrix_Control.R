...
#Results used to generate Supplementary Figure 1, Dataset 1 controls
date()
...
write.table(cont.table, "../Script_Output/Dataset1_Output/SparCC_cont_table.txt", row.names=F, quote=F, sep="\t")

cat("\n","SparCC input data created","\n")

#Calculate SparCC correlation matrices
cat("\n","Beginning SparCC correlation calculations","\n")
system("SparCC.py ../Script_Output/Dataset1_Output/SparCC_cont_table.txt --cor_file=../Script_Output/Dataset1_Output/SparCC_Corr_Matrix_Controls.txt --cov_file=../Script_Output/Dataset1_Output/SparCC_Cov_Matrix_Controls.txt")
cat("\n","Finished","\n")

#Create shuffled (with replacement) datasets for pseudo-pvalue calculation
cat("\n","Beginning construction of shuffled datasets","\n")
system("mkdir ../Script_Output/Dataset1_Output/SparCC_permutations_cont")
system("MakeBootstraps.py ../Script_Output/Dataset1_Output/SparCC_cont_table.txt -n 3000 --template=SparCC_cont_table_perm#.txt --path=../Script_Output/Dataset1_Output/SparCC_permutations_cont/")
cat("\n","Finished","\n")
...
system("cd ../Script_Output/Dataset1_Output/SparCC_permutations_cont/; for i in SparCC_cont_table_perm*.txt; do SparCC.py $i --cor_file=$i.cor.txt --cov_file=$i.cov.txt; done")
cat("\n","Finished","\n")

#Calculate pseudo-pvalues for SparCC correlations
cat("\n","Beginning pseudo-pvalue calculation for SparCC correlations","\n")
system("PseudoPvals.py ../Script_Output/Dataset1_Output/SparCC_Corr_Matrix_Controls.txt ../Script_Output/Dataset1_Output/SparCC_permutations_cont/SparCC_cont_table_perm#.txt.cor.txt 3000 -o ../Script_Output/Dataset1_Output/SparCC_Corr_Matrix_Controls_pvals.txt -t two_sided")
cat("\n","Finished","\n")

system("rm -r ../Script_Output/Dataset1_Output/SparCC_permutations_cont")
system("rm ../Script_Output/Dataset1_Output/SparCC_cont_table.txt")
